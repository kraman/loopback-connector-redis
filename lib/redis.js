...
BridgeToRedis.prototype.save = function (model, data, callback) {
    ...
    var that = this;
    async.waterfall([
      function getCurrentValues(cb) {
        that.client.hgetall(model + ':' + data.id, cb);
      },
      function updateValues(prevData, cb) {
        var watchCmd = ['WATCH', model + ':' + data.id];
        var hmsetCmd = ['HMSET', model + ':' + data.id];
        var hgetallCmd = ['HGETALL', model + ':' + data.id];
        for (i in data) {
          hmsetCmd.push(i);
          hmsetCmd.push(data[i]);
        }
        that.client.multi([watchCmd, hmsetCmd, hgetallCmd],
          function(err, result) {
            var curData = {};
            for (var i=0; i<result[1].length; i+=2){
              curData[result[1][i]] = result[1][i+1];
            }
            cb(err, curData, prevData);
          }
        );
      },
      function updateIndexes(curData, prevData, cb) {
        curData = that.fromDb(model, curData);
        prevData = that.fromDb(model, prevData);
        that.updateIndexes(
          model,
          curData.id,
          curData,
...
BridgeToRedis.prototype.updateIndexes = function (model, id, data, callback, prevData) {
    ...
    function addIndex(key, value, id){
      schedule.push([
          'SADD',
          'i:' + model + ':' + key + ':' + value,
          id
    ...
    function remIndex(key, value, id){
      schedule.push([
          'SREM',
          'i:' + model + ':' + key + ':' + value,
          id
    ...
    Object.keys(i).forEach(function (key) {
        if (data.hasOwnProperty(key)) {
            ...
            var prevVal = null;
            if (prevData) {
              prevVal = prevData[key];
            }

            if(val instanceof Array || prevVal instanceof Array) {
              val = val || [];
              prevVal = prevVal || [];

              for (j in val) {
                if (prevVal.indexOf(val[j]) < 0) {
                  addIndex(key, val[j], id);
                }
              }
              for (j in prevVal) {
                if (val.indexOf(prevVal[j]) < 0) {
                  remIndex(key, prevVal[j], id);
                }
              }
            } else {
              addIndex(key, val, id);
              if (prevVal && val !== prevVal) {
                remIndex(key, prevVal, id);
              }
            }
        }
    });
